...
class Anten:
    ...
    def Anten_Pixel(self,i,j,size_antenna_x,size_antenna_y,pixel_size):
        x = (i-size_antenna_x//2)*3*pixel_size+((j-size_antenna_y//2)%2)*3*pixel_size/2
        y = (j-size_antenna_y//2)*(np.sqrt(3)*pixel_size/2)
        par_change1 = 'With Extrude\n'\
                        ...
                        '\t.Uvector \"1.0\", \"0.0\", \"0.0\"\n'\
                        '\t.Vvector \"0.0\", \"1.0\", \"0.0\"\n'\
                        '\t.Point \"'+str(x-pixel_size)+'\", \"'+str(y)+'\"\n'\
                        '\t.LineTo \"'+str(x-pixel_size/2)+'\", \"'+str(y+pixel_size*(np.sqrt(3)/2))+'\"\n'\
                        '\t.LineTo \"'+str(x+pixel_size/2)+'\", \"'+str(y+pixel_size*(np.sqrt(3)/2))+'\"\n'\
                        '\t.LineTo \"'+str(x+pixel_size)+'\", \"'+str(y)+'\"\n'\
                        '\t.LineTo \"'+str(x+pixel_size/2)+'\", \"'+str(y-pixel_size*(np.sqrt(3)/2))+'\"\n'\
                        '\t.LineTo \"'+str(x-pixel_size/2)+'\", \"'+str(y-pixel_size*(np.sqrt(3)/2))+'\"\n'\
                        '\t.Create\n'\
                        'End With\n'
